...
class ExisteImpresoraWindows {
  ...
  async verificarImpresora(dispositivo, informacion = false) {
    let bandera = false;
    let info = "";

    try {
      const { stdout } = await execPromise(
        `${ExisteImpresoraWindows.RUTA_POWERSHELL} ${ExisteImpresoraWindows.OPCION_PARA_EJECUTAR_COMANDOS} ${ExisteImpresoraWindows.COMILLA}${ExisteImpresoraWindows.COMANDO}${ExisteImpresoraWindows.COMILLA}`
      );

      const resultado = stdout.split("\n");
...
class ImprimirEpl {
   ...
   */
  async imprimir(etiqueta, impresora, impresion = true, informacion = false) {
    try {
      const archivoTemporal = await fs.mkdtemp(`${os.tmpdir()}/Eti`);
      await fs.writeFile(`${archivoTemporal}`, etiqueta);
      ...
      if (informacion) {
        ...
        if (impresion) {
          ...
          informe += `Nota: La impresión está desactivada, para habilitarla, cambiar a '$1' en la función: -> imprimir().`;
        }
        informe +=
      ...
      console.error("Error al manejar el archivo temporal:", error);
    }
  }
...
(async () => {
  ...
  await impresora.verificarImpresora("Xprinter XP-450B", true);

  const imprimirEpl = new ImprimirEpl();
  const etiqueta = imprimirEpl.construirEtiqueta("Test Label", 2);
  console.log(etiqueta);

  await imprimirEpl.imprimir(etiqueta, "Xprinter XP-450B", false, true);
